...
namespace LibreOfficeLibrary
{
    public class DocumentConverter
    {
        ...
        /// </summary>
        public void ConvertToFile(string filePath, string targetPath, string profileLocation = null, int? timeForWaiting = null, string format = "pdf", bool copyFileToTemp = true)
        {
            ...
            if (copyFileToTemp)
            {
                ...
                var tempOutputFilePath = Path.Combine(tempDirPath, Path.GetFileNameWithoutExtension(tempFilePath) + "." + format);

                File.Copy(filePath, tempFilePath);

                var worker = new LibreOfficeWorker();
                worker.DoWork($"/C -headless -writer -convert-to {format} -outdir \"{tempDirPath}\" \"{tempFilePath}\" \"-env:UserInstallation=file:///{profilePath}/\"", timeForWaiting);
